...
const FairPrice = () => {
  ...
  const TypeFilter = (jewellType, type) => {
    if (filterdcardData?.length < 1) {
      toast.warning("Select above jewellType.. ")
      return
    }
      ...
      <div className='  py-4 px-2 '>
        ...
        <div className='grid gap-3 md:gap-6 lg:gap-9 grid-cols-2 md:grid-cols-4 lg:grid-cols-5'>
          ...
          {!isLoading && (renderFinalData)?.map((item) => (
            <Link to={`/fairPrice-details/${item?._id}`}>
              {console.log("item", item)}
              <div className='fair-price-card p-2  border h-56 md:h-64  rounded shadow cursor-pointer hover:scale-105 transition-all' >
                <img src={item?.jewellImage[0]} alt={item.jewellName} className='w-full h-[69%] md:h-40 object-cover mb-4 rounded ' />
                <h2 className='md:text-md lg:text-lg text-slate-800 font-semibold border-t-2 border-[#efe3e3]'>{item.jewellName}</h2>
                <p className='text-gray-600 text-sm md:text-md pb-1'>{item.jewellType}</p>
              </div>
            </Link>
          ))}
