...
const JewllDesignCard = ({ item }) => {
    ...
    return (
        <Link to={`/JewllDesign-details/${item?._id}`} className="  h-52 bg-white  md:h-56 lg:h-60 shadow-md border m-2 rounded-lg overflow-hidden transform transition-transform hover:scale-105">
            <img src={item?.jewellImage[0]} alt="img" className="w-full h-[145px] md:h-[150px]  lg:h-40  object-scale-down   rounded px-2 py-1" />

            <div className="px-2 py-2 md:py-3  bg-white flex">
                <div className='w-3/4'>
